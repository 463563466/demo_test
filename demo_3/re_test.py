...
db = OracleOperation()
DATA = db.execute_sql("SELECT case_id,case_reason,fmt_content FROM jugement_table_0728_rest where case_id='8a7adeaee7e0ecb32e0809acf982ade50'")
data = db.get_data()
num = 0
for concent in data:
    case_id = concent[0]
    try:
        ...
        if len(data_public_patent) > 0:
            ...
            test_str_public_num_patent = 'null'
        ...
        if len(data_public_shangbiao) > 0:
            ...
            test_str_public_num_shangbiao = 'null'
        ...
        num+=1
        with open('log1.txt','a',encoding='utf-8') as aa:
            aa.write(case_id)
            aa.write('\n')
